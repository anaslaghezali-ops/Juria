...
class TaskService extends BaseService {
  ...
  async loadTasks(orgId) {
    try {
      // Colonnes réelles : id, title, status, priority, assigned_to,
      // created_by, organization_id, folder_id, document_id, risk_id,
      // due_date, created_at
      const data = await this.getByOrg(orgId, `
        id, title, status, priority,
        assigned_to, created_by, organization_id,
        folder_id, document_id, risk_id,
        due_date, created_at
      `, {
        order: { column: 'created_at', ascending: false },
      });

      console.log('[TaskService] Chargés:', data.length);
  ...
  async createTask(payload, orgId, userId) {
    // Colonnes réelles : id, title, status, priority, assigned_to,
    // created_by, organization_id, folder_id, document_id, risk_id, due_date
    const taskData = {
      title:           payload.title,
      status:          payload.status   || 'todo',
      priority:        payload.priority || 'medium',
      assigned_to:     payload.assigned_to || payload.assignee || null,
